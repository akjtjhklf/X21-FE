...
const ManageSubject = () => {
    ...
    const handleOk = async () => {
        ...
        try {
            if (selectedSubject) {
                await axios.put(`http://localhost:3001/subjects/${selectedSubject._id}`, values);
                message.success("Chủ đề đã được cập nhật thành công!");
            } else {
                await axios.post("http://localhost:3001/subjects", values);
            ...
            {subjects.length > 0 ? (
                <div className="d-flex flex-row flex-wrap subject-main">
                    {subjects.map((subject) => (
                        <div key={subject._id} className="subject-item" onClick={() => setSelectedSubject(subject)}>
                                    ...
                                    items: [
                                        {
                                            key: '1',
                                            label: (
                                                <EditOutlined className="dropdown-item" onClick={() => showModal(subject)} />
                                            ),
                                        },
                                        {
                                            key: '2',
                                            label: (
                                                <DeleteOutlined className="dropdown-item" onClick={() => deleteSubject(subject._id)} />
                                            ),
                                        },
                                    ],
                                }}
