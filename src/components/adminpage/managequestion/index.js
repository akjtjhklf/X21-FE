...
const ManageQuestion = () => {
    ...
    return (
        <div className="col-9">
            ...
            <Modal title="Chọn loại thách thức" open={isModalVisible} onOk={handleOk} onCancel={handleCancel} width={800}>
                <Form form={form}>
                    <Form.Item name="challengeType" label="Loại thách thức">
                        <Select onChange={handleChange}>
                            <Select.Option value="">Vui lòng chọn một loại thử thách để thêm </Select.Option>
                                ...
                                <Form.Item name="subject" label="Chủ đề">
                                    <Select>
                                        {subjects.map((subject) => (
                                            <Select.Option key={subject._id} value={subject._id}>{subject.name}</Select.Option>
                                        ))}
                                    </Select>
                                </Form.Item>
                            </div>
                ...
                <div className="row questions-list">
                    {qnas.map((qna) => (
                        <div key={qna._id} className="question-item col-3">
                            <p className="question-item-content">Câu hỏi:<br />{qna.question}</p>
                            <p className="question-item-content">Cách chơi: {qna.challengeType}</p>
                                    ...
                                    items: [
                                        {
                                            key: '1',
                                            label: (
                                                <EditOutlined className="dropdown-item" onClick={() => showModal(qna)} />
                                            ),
                                        },
                                        {
                                            key: '2',
                                            label: (
                                                <DeleteOutlined className="dropdown-item" onClick={() => deleteQuestion(qna._id)} />
                                            ),
                                        },
                                    ],
                                }}
